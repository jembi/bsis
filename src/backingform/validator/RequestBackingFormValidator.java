...
public class RequestBackingFormValidator extends BaseValidator<RequestBackingForm> {
  ...
  public void validateForm(RequestBackingForm form, Errors errors) throws Exception {
    ...
    commonFieldChecks(form, "request", errors);
  }

  private void updateAutogeneratedFields(RequestBackingForm form) {
    if (StringUtils.isBlank(form.getRequestNumber()) && isFieldAutoGenerated("request", "requestNumber")) {
      form.setRequestNumber(sequenceNumberRepository.getNextRequestNumber());
    }
    if (StringUtils.isBlank(form.getRequestDate()) && doesFieldUseCurrentTime("request", "requestDate")) {
