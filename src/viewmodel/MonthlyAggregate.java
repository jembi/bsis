...
public class MonthlyAggregate {
    ...
    public static MonthlyAggregate createWithCollections(ArrayList<Collection> collections) {
        MonthlyAggregate monthlyAggregate = new MonthlyAggregate();
        monthlyAggregate.aggregates = new TreeMap<Date, Integer>(new Comparator<Date>() {
        ...
        List<Date> dateList = (List<Date>) CollectionUtils.collect(collections, new Transformer() {
            public Object transform(Object o) {
                ...
                DateTime firstDayOfMonth = dateTime.minusDays(dateTime.getDayOfMonth() - 1);
        ...
        for (Date uniqueDate : uniqueDates) {
            ...
            monthlyAggregate.aggregates.put(uniqueDate, dailyCollections.size());
        }
        return monthlyAggregate;
    }

    public static MonthlyAggregate createWithProducts(ArrayList<Product> filteredProducts) {
        MonthlyAggregate monthlyAggregate = new MonthlyAggregate();
        monthlyAggregate.aggregates = new TreeMap<Date, Integer>(new Comparator<Date>() {
            public int compare(Date date, Date date1) {
                return date.compareTo(date1);
    ...
    public static MonthlyAggregate createWithTestResults(ArrayList<TestResult> filteredTestResults) {
        MonthlyAggregate monthlyAggregate = new MonthlyAggregate();
        monthlyAggregate.aggregates = new TreeMap<Date, Integer>(new Comparator<Date>() {
            public int compare(Date date, Date date1) {
                return date.compareTo(date1);
        ...
        for (Date uniqueDate : uniqueDates) {
            final Date collectionDate = uniqueDate;
            List<TestResult> dailyProducts = (List<TestResult>) CollectionUtils.select(filteredTestResults, new Predicate() {
                public boolean evaluate(Object o) {
                    TestResult testResult = (TestResult) o;
                    DateTime dateTime = new DateTime(testResult.getDateCollected());
                    DateTime firstDayOfMonth = dateTime.minusDays(dateTime.getDayOfMonth() - 1);
    ...
    public LinkedHashMap<Date, Integer> getFilledMonthlyAggregate(LinkedHashMap<Date, Integer> orderedAggregates) {
