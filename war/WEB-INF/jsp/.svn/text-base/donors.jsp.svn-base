...
<c:if test="${model.donorNotFound==true}">
    <div id="donorErrorMessagePanel" class="message">
        This donor is not present in records. Please create a new entry.
    </div>
</c:if>
...
<form:form action="findDonor.html" id="donorAction">
    <div id="donorDetails">
        <c:if test="${model.hasDonor==true}"><input id="donorId" name="donorId" type="hidden"
                                                    value="${model.donor.donorId}"/></c:if>

        <div class="inputFieldRow"><label for="donorNumber">${model.donorIDDisplayName}: </label><input req="true" type="text"
                                                                                                        maxlength="32"
                                                                                                        id="donorNumber"
                                                                                                        name="donorNumber"
        ...
        <c:if test="${model.showfirstName==true}">
            ...
            <div class="inputFieldRow"><label for="firstName">${model.firstNameDisplayName}: </label><input req="true" type="text"
                                                                                                            maxlength="32"
                                                                                                            id="firstName"
                                                                                                            name="firstName"
        ...
        <c:if test="${model.showlastName==true}">
            ...
            <div class="inputFieldRow"><label for="lastName">${model.lastNameDisplayName}: </label><input type="text"
                                                                                                          id="lastName"
                                                                                                          maxlength="32"
                                                                                                          name="lastName"
        ...
        <c:if test="${model.showgender==true}">
            ...
            <div class="createDonorField inputFieldRow"><label for="gender">${model.genderDisplayName}: </label>
        ...
        <c:if test="${model.showbloodType==true}">
            ...
            <div class="createDonorField inputFieldRow"><label for="bloodType">${model.bloodTypeDisplayName}: </label>
        ...
        <c:if test="${model.showdateOfBirth==true}">
            ...
            <div class="createDonorField inputFieldRow"><label for="dobmonth">${model.dobDisplayName}: </label>

                <input type="text" maxlength="2" style="width:30px;" id="dobmonth" name="dobmonth"
                       <c:if test="${model.hasDonor==true}">value="${model.donor.birthDateMonth}"</c:if>

                        /> /
                <input type="text" maxlength="2" style="width:30px;" id="dobday" name="dobday"
                       <c:if test="${model.hasDonor==true}">value="${model.donor.birthDateDay}"</c:if>

                        /> /
                <input type="text" maxlength="4" style="width:50px;" id="dobyear" name="dobyear"
                       <c:if test="${model.hasDonor==true}">value="${model.donor.birthDateYear}"</c:if>

                        />

                <div class="infoMessage" style="margin: 0px; padding: 0px;">
                    <label style="width:40px;margin: 2px 0 0 3px;"> mm / </label>
                    <label style="width:30px;margin: 2px 0 0 17px;"> dd / </label>
                    <label style="width:50px;margin: 2px 0 0 25px;"> yyyy </label>
                </div>
            </div>
            <div class="createDonorField inputFieldRow" style="clear:both;padding-bottom: 20px;"><label>OR</label></div>
        </c:if>
        <c:if test="${model.showage==true}">

            <div class="createDonorField inputFieldRow"><label for="age">${model.ageDisplayName}: </label><input
                    type="text"
                    numOnly="true"
                    id="age"
                    maxlength="2"
        ...
        <c:if test="${model.showaddress==true}">
            ...
            <div class="createDonorField inputFieldRow"><label for="address">${model.addressDisplayName}: </label><input
                    type="text"
                    id="address"
                    maxlength="64"
                    name="address"
    ...
    <div id="findDonorButtonPanel" class="actionButtonsPanel">
        <input
                id="findDonorButton" type="submit" value="Find Donor"/>
    </div>

    <div class="actionButtonsPanel" id="createDonorButtonPanel"><input id="createDonorButton"
                                                                       type="button"
                                                                       value="Create Donor"/>
    ...
    <div class="actionButtonsPanel" id="updateDonorButtonPanel"><input id="updateDonorButton"
                                                                       type="button"
                                                                       value="Update Donor"/>
        <input id="deleteDonorButton"
               type="button"
               value="Delete Donor"/>
        <a class="link backToFindDonor">Back</a>
    </div>
...
<c:if test="${model.multipleDonorsFound==true}">
    <div id="donorChoices">
        <form:form action="selectDonor.html" id="donorTableAction">
            <div class="subHeading" style="margin-top:30px;"><p>Possible Donors</p></div>
...
<c:if test="${model.donorHistory!=null}">
    <div id="donorHistory">
        <div id="donorHistoryHeadingPanel" class="collapsibleHeadingPanel" title="Click to expand/collapse">
            <p id="expandButton"
               class="expandButton">
                <img
                        id="donorHistoryPanelButton"
                        src="images/plus.png"/>
            </p>

            <p class="collapsibleHeadingPanelText">Donor History</p></div>
